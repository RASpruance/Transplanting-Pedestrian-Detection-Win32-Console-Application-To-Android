...
// Add this to Android.mk: LOCAL_LDLIBS += -llog
...
bool IntChnPedestrianDetect::InitDetect(int m_ImageWidth, int m_ImageHeight, bool m_FlagWhole) {
    ...
    vector<string> m_MultiSplitThetaPath;
    ...
    if (m_FlagWhole) {
        ...
        m_MultiSplitThetaPath.push_back(m_SplitThetaPath1);
        m_MultiSplitThetaPath.push_back(m_SplitThetaPath2);
        m_MultiSplitThetaPath.push_back(m_SplitThetaPath3);
        ...
        m_MultiSplitThetaPath.push_back(m_HalfSplitThetaPath1);
        m_MultiSplitThetaPath.push_back(m_HalfSplitThetaPath2);
        m_MultiSplitThetaPath.push_back(m_HalfSplitThetaPath3);
    ...
    if (!InitDetectParam(m_MultiModelPath, m_MultiSplitThetaPath, m_MultiModelSize, m_ImageWidth,
                         m_ImageHeight, 3, m_MultiTheta, 1.2)) {
...
bool IntChnPedestrianDetect::InitDetectParam(vector<string> MultiModelPath,
                                             vector<string> MultiSplitThetaPath,
                                             vector<CascadeModelSize> MultiModelSize,
                                             int m_ImageWidth, int m_ImageHeight, int MinNeighbors,
                                             vector<double> DetaTheta, double DetectScale) {
    ...
    if (!LoadCasacadeModel(m_OriMultiCascadeClassfiers, MultiModelPath)) {
        return false;
    }
    LOGD("%s", "checkpoint 3.1.2");
    vector<vector<double> > m_OriMultiSplitTheta;
    if (!LoadMultiSplitTheta(m_OriMultiSplitTheta, MultiSplitThetaPath))
        return false;

    for (int i = (int) (m_OriModelsSize.size() - 1); i >= 0; i--) {
        if (m_OriModelsSize[i].width >= m_ImageWidth ||
            m_OriModelsSize[i].height >= m_ImageHeight) {
    ...
    m_DetectResizeNum = 0;
    int m_MultiModelNum = (int) m_OriModelsSize.size();
    for (int i = 0; i < m_MultiModelNum - 1; i++) {
        for (int m_DetectIdx = 0; m_DetectModelWidth < m_OriModelsSize[i + 1].width &&
                                  m_DetectModelHeight <
                                  m_OriModelsSize[i + 1].height; m_DetectIdx++) {
            m_DetectResizeNum++;
            ...
            for (vector<StrongClassfier>::size_type n = 0; n < m_TempCascadeClassfier.size(); n++) {
                ...
                for (vector<WeakClassfier>::size_type n1 = 0;
                     n1 < m_TempCascadeClassfier[n].weakclassfierset.size(); n1++) {
                    WeakClassfier m_TempWeakClassfier = m_TempCascadeClassfier[n].weakclassfierset[n1];
                    m_TempWeakClassfier.f.x = (int) (m_ResizeScale * m_TempWeakClassfier.f.x);
                    m_TempWeakClassfier.f.y = (int) (m_ResizeScale * m_TempWeakClassfier.f.y);
                    m_TempWeakClassfier.f.dx = (int) (m_ResizeScale * m_TempWeakClassfier.f.dx);
                    m_TempWeakClassfier.f.dy = (int) (m_ResizeScale * m_TempWeakClassfier.f.dy);

                    if (m_TempWeakClassfier.f.type > 0 && m_TempWeakClassfier.f.type < 4) {
                        m_TempWeakClassfier.realthres =
                                m_ResizeScale * m_ResizeScale * m_TempWeakClassfier.realthres;
                    }
                    else {
                ...
                m_TempStrongClassfier.rawth = m_TempCascadeClassfier[n].rawth +
                                              m_DetaTheta[i] * abs(m_TempCascadeClassfier[n].rawth);
            ...
            m_DetectModelWidth = (int) (m_ResizeScale * m_OriModelsSize[i].width + 0.5);
            m_DetectModelHeight = (int) (m_ResizeScale * m_OriModelsSize[i].height + 0.5);
        }
    ...
    for (int m_DetectIdx = 0;
         m_DetectModelWidth < m_ImageWidth && m_DetectModelHeight < m_ImageHeight; m_DetectIdx++) {
        m_DetectResizeNum++;
        ...
        for (vector<StrongClassfier>::size_type n = 0; n < m_TempCascadeClassfier.size(); n++) {
            ...
            m_TempStrongClassfier.rawth = m_TempCascadeClassfier[n].rawth +
                                          m_DetaTheta[m_MultiModelNum - 1] *
                                          abs(m_TempCascadeClassfier[n].rawth);
        ...
        m_ResizeScale *= m_DetectScale;
        m_DetectModelWidth = (int) (m_ResizeScale * m_OriModelsSize[m_MultiModelNum - 1].width +
                                    0.5);
        m_DetectModelHeight = (int) (m_ResizeScale * m_OriModelsSize[m_MultiModelNum - 1].height +
                                     0.5);
    }
    return true;
}
...
void IntChnPedestrianDetect::InitMemoryMalloc(int m_ImageWidth, int m_ImageHeight) {
    ...
    m_ChannelsImage.data = (float *) malloc(
            sizeof(float) * m_ChannelsImage.width * m_ChannelsImage.height *
            m_ChannelsImage.nChannels);
    memset(m_ChannelsImage.data, 0, sizeof(float) * m_ChannelsImage.width * m_ChannelsImage.height *
                                    m_ChannelsImage.nChannels);

    m_OriImage.width = m_ImageWidth;
    m_OriImage.height = m_ImageHeight;
    m_OriImage.nChannels = 3;
    m_OriImage.data = (float *) malloc(
            sizeof(float) * m_OriImage.width * m_OriImage.height * m_OriImage.nChannels);
    memset(m_OriImage.data, 0,
           sizeof(float) * m_OriImage.width * m_OriImage.height * m_OriImage.nChannels);
    ...
    m_SumImage.data = (float *) malloc(
            sizeof(float) * m_SumImage.width * m_SumImage.height * m_SumImage.nChannels);
    memset(m_SumImage.data, 0,
           sizeof(float) * m_SumImage.width * m_SumImage.height * m_SumImage.nChannels);
    ...
    m_Sum2Image.data = (float *) malloc(
            sizeof(float) * m_Sum2Image.width * m_Sum2Image.height * m_Sum2Image.nChannels);
    memset(m_Sum2Image.data, 0,
           sizeof(float) * m_Sum2Image.width * m_Sum2Image.height * m_Sum2Image.nChannels);
...
int IntChnPedestrianDetect::DetectImage(TxtChnImage &m_Image, TRect *pRect) {
    ...
    for (int m_DetectIdx = 0; m_DetectIdx < m_DetectResizeNum; m_DetectIdx++) {
        ...
        for (m_PointY = 0; (m_PointY + m_DetectSizeY) < m_ImageHeight; m_PointY += m_DetectStep) {
            ...
            for (m_PointX = 0;
                 (m_PointX + m_DetectSizeX) < m_ImageWidth; m_PointX += m_DetectStep) {
                z = 0;
                ...
                bool IsObjectFlag = IsExistObjectInScaleCascade(m_SumImage, m_PointX + 1,
                                                                m_PointY + 1);
                if (IsObjectFlag) {
...
void IntChnPedestrianDetect::CalChannelsFeature(TxtChnImage &m_OriImage,
    ...
    float *pGradHist =
            m_ChannelsImage.data + m_ChannelsImage.width * m_ChannelsImage.height * 4;
    LOGD("m_ImageWidth=%d, m_ImageHeight=%d, m_ImageChannels=%d", m_ImageWidth, m_ImageHeight,
         m_ImageChannels);
    ...
    gradHist(pGradMag, pGradOrient, pGradHist, m_ChannelsImage.width, m_ChannelsImage.height, 1, 6,
             false);
    LOGD("gradHist succeeded");
}

void IntChnPedestrianDetect::CalChannelsFeaIntegral(const TxtChnImage &m_ChannelsFea,
...
void IntChnPedestrianDetect::CalChannelsFeaIntegral(TxtIntegralImage &m_IntegralImage,
...
bool IntChnPedestrianDetect::IsExistObjectInScaleCascade(TxtIntegralImage &m_Image, int m_PointX,
                                                         int m_PointY) {
    ...
    int m_WeakCount = (int) m_CascadeClassfier[0].weakclassfierset.size();
    ...
    return h >= m_CascadeClassfier[0].rawth;
}

double IntChnPedestrianDetect::StrongClassValue(StrongClassfier &sc, TxtIntegralImage &m_Image,
                                                int m_PointX, int m_PointY) {
    double h = 0;
    int m_WeakCount = (int) sc.weakclassfierset.size();
    for (int i = 0; i < m_WeakCount; i++)
        h += WeakClassValue(sc.weakclassfierset[i], m_Image, m_PointX, m_PointY);
    return h;
}

double IntChnPedestrianDetect::WeakClassValue(WeakClassfier &wc, TxtIntegralImage &m_Image,
                                              int m_PointX, int m_PointY) {
    double val = FeatureValue(wc.f, m_Image, m_PointX, m_PointY);

    if (wc.f.type > 0 && wc.f.type < 4) {
        if (val >
            wc.realthres * m_Image.var[wc.f.type] + wc.f.dx * wc.f.dy * m_Image.avg[wc.f.type])
            return wc.alphaR;
        else
            return wc.alphaL;
    }
...
double IntChnPedestrianDetect::FeatureValue(ChnFeature &f, TxtIntegralImage &m_Image, int m_PointX,
                                            int m_PointY) {
    int x = f.x + m_PointX;
    int y = f.y + m_PointY;
    int dx = f.dx;
...
bool IntChnPedestrianDetect::LoadMultiSplitTheta(vector<vector<double> > &m_MultiSplitTheta,
                                                 vector<string> m_SplitThetaPath) {
...
bool IntChnPedestrianDetect::LoadCasacadeModel(
        vector<vector<StrongClassfier> > &m_MultiModelClassfier,
        vector<string> m_CasacadeMultiModelFile) {
...
bool IntChnPedestrianDetect::RectIsEqual(TRect &m_Rect1, TRect &m_Rect2) {
    ...
    float area = (m_Rect1.iBr.iX - m_Rect1.iTl.iX + 1) * (m_Rect1.iBr.iY - m_Rect1.iTl.iY + 1) +
                 (m_Rect2.iBr.iX - m_Rect2.iTl.iX + 1) * (m_Rect2.iBr.iY - m_Rect2.iTl.iY + 1) -
                 overlap;
    return (overlap / area < 0.5f);
}

int IntChnPedestrianDetect::RectPartition(TRect *pDstRect, TRect *pOriRect, int m_DetectNum,
                                          int m_MinNeighbors) {
    ...
    while (1) {
        ...
        for (int i = 0; i < m_DetectNum; i++) {
            ...
            for (int j = 0; j < m_DetectNum; j++) {
                if (pFlagState[j] || j == i)
                    continue;
                if (!RectIsEqual(pOriRect[i], pOriRect[j]))
                    continue;
                pNumOverlap[i]++;
            }
        }
        ...
        for (int i = 0; i < m_DetectNum; i++) {
            if (pFlagState[i])
                continue;
            if (RectIsEqual(pOriRect[m_MaxIdx], pOriRect[i])) {
...
void IntChnPedestrianDetect::gradQuantize(float *O, float *M, int *O0, int *O1, float *M0,
                                          float *M1, int nOrients, int nb, int n, float norm) {
    // assumes all *OUTPUT* matrices are 4-byte aligned
    int i, o0, o1;
    float o, od, m;
    // define useful constants
    const float oMult = (float) nOrients / PI;
...
void IntChnPedestrianDetect::gradHist(float *M, float *O, float *H, int h, int w, int bin,
                                      int nOrients, bool softBin) {
...
float *IntChnPedestrianDetect::rgb2luv_setup(float z, float *mr, float *mg, float *mb, float &minu,
...
void m_BtnInitParam_Click(PhoneComPedestrianDetect::IntChnPedestrianDetect *m_PedestrianDetect,
                          PtrToParam MyParam) {
    bool m_FlagState = m_PedestrianDetect->InitDetect(MyParam->m_ImageWidth, MyParam->m_ImageHeight,
                                                      1);
    if (!m_FlagState) {
        LOGD("Param Error");
    }
...
JNIEXPORT jintArray JNICALL Java_com_bn_ex7f_PedDetect_Start_1Detect
    ...
    for (int i = 1; i < NumOfBoundingBox_Yuxuan + 1; i++)
        LOGD("%d,%d,%d,%d", Result[4 * i - 3],
             Result[4 * i - 2],
             Result[4 * i - 1], Result[4 * i]);
    ...
    env->SetIntArrayRegion(result, 0, NumOfBoundingBox_Yuxuan * 4 + 1,
                           NumWithFourPointsForBoundingBoxes);
